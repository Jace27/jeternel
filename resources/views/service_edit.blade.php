...
@section('title')
Редактировать услугу - {{ \App\Settings::$title_site_name }} @endsection

@section('body')
    <h2>Редактировать услугу</h2>
    <form action="/api/service/{{ $id }}/edit" method="post" enctype="multipart/form-data" name="edit_service_form">
        <?php
        $service = \App\Models\services::withTrashed()->where('id', $id)->first();
        if ($service == null){
            echo '<h3>Услуги не существует</h3>';
        } else {
        $cats = $service->categories()->get();
        ?>
            <h4>Название:</h4>
            <input type="text" name="name" class="form-control input" value="{{ $service->name }}" />
            ...
            <div class="border mt-3 mb-3 p-2">
                ...
                <div id="class_categories">
                    @foreach($cats as $cat)
                    @if($cat->type()->first()->name == 'По классу')
                        <div class="pt-1 pb-1">
            ...
            <div class="border mt-3 mb-3 p-2">
                ...
                <hr>
                <div id="other_names">
                    @foreach($service->other_names()->get() as $other_name)
                        <div class="pt-1 pb-1">
                            <input type="text" class="form-control select other_name mr-1" style="max-width: calc(100% - 95px)" name="other_name{{ $other_name->id }}" value="{{ $other_name->other_name }}">
            ...
            <textarea name="instruct1" class="form-control input">{!! $service->instruct1 !!}</textarea>
            ...
            <div class="border mt-3 mb-3 p-2">
                <table class="table-borderless">
                    ...
                    <tr>
                        ...
                        <td><input type="number" style="max-width: 120px;" step="0.01" value="{{ $service->price()->first()->nonvip_low }}" min="0" name="price_nonvip_low" class="form-control d-inline-block input">&nbsp;руб.</td>
                    </tr>
                    <tr>
                        <td>Филиалы, врач:</td>
                        <td><input type="number" style="max-width: 120px;" step="0.01" value="{{ $service->price()->first()->nonvip_high }}" min="0" name="price_nonvip_high" class="form-control d-inline-block input">&nbsp;руб.</td>
                    </tr>
                    <tr>
                        <td>В13, косметолог:</td>
                        <td><input type="number" style="max-width: 120px;" step="0.01" value="{{ $service->price()->first()->vip_low }}" min="0" name="price_vip_low" class="form-control d-inline-block input">&nbsp;руб.</td>
                    </tr>
                    <tr>
                        <td>В13, врач:</td>
                        <td><input type="number" style="max-width: 120px;" step="0.01" value="{{ $service->price()->first()->vip_high }}" min="0" name="price_vip_high" class="form-control d-inline-block input">&nbsp;руб.</td>
                    </tr>
                    </tbody>
                </table>
            </div>

            <textarea name="instruct2" class="form-control input">{!! $service->instruct2 !!}</textarea>

            <div class="border mt-3 mb-3 p-2">
                <h4>Сопутствующие акции:</h4>
                <hr>
                <div id="promotions">
                    @foreach($service->promotions()->get() as $prom)
                        <div class="pt-1 pb-1">
                            <input type="text" disabled class="form-control select mr-1" style="max-width: calc(100% - 95px)" value="{{ $prom->promotion()->first()->title }}">
                            <input type="hidden" class="promotion" value="{{ $prom->promotion()->first()->id }}">
                ...
                <select id="select_promotion_add" class="form-control select" style="max-width: calc(100% - 110px)">
                    <option selected value="null">Выбрать...</option>
                    @foreach(\App\Models\promotions::all() as $promotion)
            ...
            <hr>
            <h4>Описание:</h4>
            <textarea name="description" id="description" class="input">{!! $service->description !!}</textarea>

            <hr>
            <h4>Подготовка:</h4>
            <textarea name="preparation" id="preparation" class="input">{!! $service->preparation !!}</textarea>

            <hr>
            <h4>Реабилитация:</h4>
            <textarea name="rehabilitation" id="rehabilitation" class="input">{!! $service->rehabilitation !!}</textarea>

            <hr>
            <h4>Показания:</h4>
            <textarea name="indications" id="indications" class="input">{!! $service->indications !!}</textarea>

            <hr>
            <h4>Противопоказания:</h4>
            <textarea name="contraindications" id="contraindications" class="input">{!! $service->contraindications !!}</textarea>

            <hr>
            <h4>Курс:</h4>
            <textarea name="course" id="course" class="input">{!! $service->course !!}</textarea>
            ...
            <div class="border mt-3 mb-3 p-2">
                ...
                <div id="drugs">
                    ...
                    @foreach($service->drugs()->get() as $drug)
                        <div class="pt-1 pb-1" id="drug-{{ $i }}">
                            <input type="text" disabled class="form-control select mr-1 drug-name" style="max-width: calc(100% - 410px)" value="{{ $drug->drug()->first()->name }}&nbsp;{{ $drug->drug()->first()->manufacturer }}">
                            <input type="text" class="form-control select mr-1 drug-volume" style="max-width: 200px" value="{{ $drug->using_volume }}">
                            <input type="hidden" class="drug" value="{{ $drug->drug()->first()->id }}">
            ...
            <div class="border mt-3 mb-3 p-2">
                ...
                <div id="performers">
                    @foreach($service->performers()->get() as $performer)
                        <div class="pt-1 pb-1" id="performer-{{ $performer->performer()->first()->id }}">
                            <input type="text" disabled class="form-control select mr-1 performer-name" style="max-width: calc(100% - 310px)" value="{{ $performer->performer()->first()->last_name }}&nbsp;{{ $performer->performer()->first()->first_name }}&nbsp;{{ $performer->performer()->first()->second_name }}">
                            <input type="time" class="form-control select mr-1 performer-time" style="max-width: 100px" value="{{ $performer->duration }}">
                            <input type="hidden" class="performer" value="{{ $performer->performer()->first()->id }}">
                ...
                <select class="form-control select" style="max-width: calc(100% - 110px)" id="select-performer-add">
                    ...
                    @foreach(\App\Models\performers::all() as $performer)
                        <option value="{{ $performer->id }}">{{ $performer->last_name }}&nbsp;{{ $performer->first_name }}&nbsp;{{ $performer->second_name }}
                        <ul>
                            @foreach($performer->branches()->get() as $branch)
                                <li>{{ $branch->address }}<br>{{ $branch->name }}</li>
                            @endforeach
                        </ul>
                        </option>
                    @endforeach
            ...
            <br>
            <center><input type="submit" value="Сохранить" class="btn btn-outline-primary btn-service-save"></center>
        <?php
        }
        ?>
    </form>
...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            let last_errors = [];
            let validator = new FormValidator('edit_service_form', [
                {
                    name: 'name',
                    display: 'Название',
                    rules: 'required'
            ...
            $('form[name=edit_service_form]').submit(function(e) {
                e.preventDefault();
            });
            ...
            $('.btn-service-save').click(function(e){
                ...
                if ($('input[name=name]').val().trim() != ''){
                    ...
                    $.ajax({
                        url: '/api/service/{{ $service->id }}/edit',
                        method: 'post',
                        data: data,
                        processData: false,
                        contentType: false,
